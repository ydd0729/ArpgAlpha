...
namespace ArchieAndrews.PrefabBrush
{
    ...
    public class PrefabBrush : EditorWindow
    {
        ...
        //Display the window.
        [MenuItem("Tools/Prefab Brush+(地形批量投放预制体工具)")]
...
#region GUI
        ...
        private void DrawPrefabPaintTab()
        {
            ...
            {
                EditorGUILayout.Space();
                showBrushSettings = EditorGUILayout.Foldout(showBrushSettings, "笔刷设置", styleFold);
                ...
                showObjectSettings = EditorGUILayout.Foldout(showObjectSettings, "对象设置", styleFold);
            ...
            {
                EditorGUILayout.HelpBox("预设画笔关闭", MessageType.Warning);
            }
        ...
        private void DrawPhysicsPaintSettings()
        {
            ...
            GUILayout.Label("物理画笔", style);
            EditorGUILayout.BeginVertical("box");
            activeSave.spawnHeight = EditorGUILayout.FloatField("绘制高度", activeSave.spawnHeight);
            activeSave.addRigidbodyToPaintedPrefab = EditorGUILayout.Toggle("将刚体添加到预设", activeSave.addRigidbodyToPaintedPrefab);
            activeSave.physicsIterations = EditorGUILayout.FloatField("物理迭代", activeSave.physicsIterations);
            EditorGUILayout.EndVertical();
        }
...
#else
            EditorGUILayout.BeginVertical("box");
            activeSave.paintType = (PB_PaintType)EditorGUILayout.EnumPopup("预制涂料类型", activeSave.paintType);
            EditorGUILayout.EndVertical();
#endif
        }
        ...
        private void DrawPrefabDisplay(PB_PrefabDataType dataType)
        {
            ...
            prefabDisplayType = (PB_PrefabDisplayType)EditorGUILayout.EnumPopup(string.Format("{0} 显示类型", dataTitle), prefabDisplayType);
        ...
        private void DrawBrushSizeSlider()
        {
            EditorGUILayout.Space();
            //Define radius of the brush.
            GUILayout.Label("笔刷大小", style);
            EditorGUILayout.BeginVertical("box");
            EditorGUILayout.BeginHorizontal();
            activeSave.笔刷大小 = EditorGUILayout.Slider(activeSave.笔刷大小, activeSave.minBrushSize, activeSave.maxBrushSize);
            EditorGUILayout.EndHorizontal();
            showMaxBrushSizeSlider = EditorGUILayout.Foldout(showMaxBrushSizeSlider, "最大滑块大小");
        ...
        private void DrawPrefabPerStrokeSlider()
        {
            ...
            showMaxMinPaintDelta = EditorGUILayout.Foldout(showMaxMinPaintDelta, "显示最大/最小绘制增量距离");

            if (showMaxMinPaintDelta)
            {
                activeSave.maxPaintDeltaDistance = EditorGUILayout.FloatField("最大绘制增量距离", activeSave.maxPaintDeltaDistance);
                activeSave.minPaintDeltaDistance = EditorGUILayout.FloatField("最小绘制增量距离", activeSave.minPaintDeltaDistance);
            }
            EditorGUILayout.EndVertical();

            EditorGUILayout.Space();
            GUILayout.Label("每冲程预制件", style);
            EditorGUILayout.BeginVertical("box");
            ...
            showMaxMinPrefabsPerStroke = EditorGUILayout.Foldout(showMaxMinPrefabsPerStroke, "显示每笔划的最大/最小预设");

            if (showMaxMinPrefabsPerStroke)
            {
                activeSave.maxprefabsPerStroke = EditorGUILayout.IntField("每冲程最大预制件", activeSave.maxprefabsPerStroke);
                activeSave.minprefabsPerStroke = EditorGUILayout.IntField("每冲程最小预制件", activeSave.minprefabsPerStroke);
            }
        ...
        {
            EditorGUILayout.Space();
            GUILayout.Label("到笔刷的倾斜角度", style);
            ...
            if (activeSave.checkSlope)
            {
                ...
                EditorGUILayout.BeginHorizontal("box");
                GUILayout.Label(string.Format("最小角度 = {0} | 最大角度 = {1}", Mathf.Round(activeSave.minRequiredSlope * 100f) / 100f, Mathf.Round(activeSave.maxRequiredSlope * 100f) / 100f), style);
                EditorGUILayout.EndHorizontal();
        ...
        {
            GUILayout.Label("预制件的偏移旋转", style);
            EditorGUILayout.BeginVertical("box");
        ...
        private void DrawParentOptions()
        {
            ...
            switch (activeSave.parentingStyle)
            {
                case PB_ParentingStyle.Surface:
                    EditorGUILayout.HelpBox("绘制的预设现在会将它们的自拍设置为绘制表面的父对象.", MessageType.Info);
                    break;
                case PB_ParentingStyle.SingleTempParent:
                    ...
                    EditorGUILayout.HelpBox("父值将不会跨场景保存", MessageType.Warning);
                    break;
        ...
        {
            GUILayout.Label("旋转游戏对象以匹配曲面", style);
            EditorGUILayout.BeginVertical("box");
            ...
            if (activeSave.rotateToMatchSurface)
            {
                ...
                EditorGUILayout.HelpBox("如果您的游戏对象没有朝向正确的方向，请尝试更改上面列出的方向。", MessageType.Info);
            }
        ...
        {
            activeSave.minScale = EditorGUILayout.FloatField("最小比例", activeSave.minScale);
            activeSave.maxScale = EditorGUILayout.FloatField("最大比例", activeSave.maxScale);
        }
        ...
        private void DrawScaleError()
        {
            if (activeSave.minXScale == 0)
                EditorGUILayout.HelpBox("最小刻度X等于0，这可能会导致预制件出现问题", MessageType.Error);

            if (activeSave.minYScale == 0)
                EditorGUILayout.HelpBox("最小刻度Y等于0，这可能会导致预制件出现问题", MessageType.Error);

            if (activeSave.minZScale == 0)
                EditorGUILayout.HelpBox("最小刻度Z等于0，这可能会导致预制件出现问题", MessageType.Error);

            if (activeSave.maxXScale == 0)
                EditorGUILayout.HelpBox("最大刻度X等于0，这可能会导致预制件出现问题", MessageType.Error);

            if (activeSave.maxYScale == 0)
                EditorGUILayout.HelpBox("最大刻度Y等于0，这可能会导致预制件出现问题", MessageType.Error);

            if (activeSave.maxZScale == 0)
                EditorGUILayout.HelpBox("最大刻度Z等于0，这可能会导致预制件出现问题", MessageType.Error);
        }
        ...
        private void DrawParentList()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            GUILayout.Label("拖放到此处可将家长添加到列表", styleBold);
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndVertical();
        ...
        private void DrawEraseDetectionType()
        {
            ...
            switch (activeSave.eraseDetection)
            {
                case PB_EraseDetectionType.Collision:
                    ...
                    EditorGUILayout.HelpBox("这将不适用于没有碰撞器组件的对象", MessageType.Warning);
                    break;
                case PB_EraseDetectionType.Distance:
                    EditorGUILayout.HelpBox("距离将适用于不使用碰撞器的对象。虽然速度很慢", MessageType.Warning);
                    EditorGUILayout.HelpBox("尽量不要与复杂的场景一起使用！禁用尽可能多的对象会有所帮助。", MessageType.Error);
                    break;
            }
        }
            ...
            switch (activeSave.eraseType)
            {
                case PB_EraseTypes.PrefabsInBrush:
                    ...
                    if (activeSave.mustBeSelectedInBrush)
                        EditorGUILayout.HelpBox("只有在保存文件中定义和选择的预制件才会被删除", MessageType.Info);
                    else
                        EditorGUILayout.HelpBox("只有在保存文件中定义的预制件才会被删除", MessageType.Info);

                    EditorGUILayout.EndVertical();
                    break;
                case PB_EraseTypes.PrefabsInBounds:
                    EditorGUILayout.HelpBox("只有符合边界的预制件才会被删除", MessageType.Info);
                    break;
            }
        }
        ...
        {
            EditorGUILayout.Space();
            GUILayout.Label("要删除的倾斜角度", style);
...
#region Methods
        ...
        private void RunPrefabPaint()
        {
            ...
            float newBrushSize = (activeSave.paintType == PB_PaintType.Single) ? 0 : activeSave.笔刷大小 * .5f;
            ...
            if (isOn && activeSave != null)
            {
                switch (activeTab)
                {
                    case PB_ActiveTab.PrefabPaint:
                        ...
                        DrawPaintCircle(placeBrush, activeSave.笔刷大小);
                        RunPrefabPaint();
                        break;
                    case PB_ActiveTab.PrefabErase:
                        HandleUtility.AddDefaultControl(GUIUtility.GetControlID(FocusType.Passive));
